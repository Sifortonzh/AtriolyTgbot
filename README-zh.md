...
Wanatring 现在在私聊中充当全功能的 **客户支持代理**。

### 工作流
1.  **用户私信**：当用户私聊机器人时，**AI 分类器**会分析文本。
2.  **标签标注**：为消息分配类别（如 `#billing`, `#membership`）并生成摘要。
3.  **转发**：处理后的消息会带着结构化抬头转发给管理员。
4.  **回复桥接**：管理员只需在 Telegram 中**回复**这条转发消息，机器人会匿名把文字回传给原用户。

### 模式 (`/mode`)
* **转发模式**（默认）：消息转发给管理员，由人工处理。
* **聊天模式**（实验）：AI 尝试自动回复常见问题，同时仍会通知管理员。
